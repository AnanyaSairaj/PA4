...
void new_get(int sock, char *path, char *file_name) {
	...
	printf("File part requested is= %d\n", file_part);
	...
	recv(sock, &request, sizeof(request), 0);
        printf("the received request value is %d",request);
...
void put(int sock,char *path, packet_t packet)
{ FILE *fptr;

char partname[30] = {0};
			sprintf(partname, "%s%s.%d", path, packet.filename, packet.file_part+1);
			printf("partname - %s\n",partname);
			...
			while (total_bytes != packet.file_size)
			{
				...
				if((numRead = recv(sock, recv_buf, packet.file_size, 0)) > 0)
				{
					total_bytes += numRead;
					...
					int msg_recv = 1;
					send(sock, &msg_recv, sizeof(int), 0);
				}
			}
			free(recv_buf);
...
{
	if ((argc < 4) || (argc > 4)) {				
		printf("Usage --> ./[%s][Local Folder][Port Number][local-directory]\n", argv[0]);	//Should have a port number > 5000
		exit(EXIT_FAILURE);
	}
