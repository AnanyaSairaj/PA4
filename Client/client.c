...
/*-----------------------Global Variable-----------------------*/
server_cred credential;//initializes credentials for the server
server_config server[4];//intializes the server as http server
...
unsigned char file_md5_counter(char *filename,size_t filesize, FILE *f)
{
  ...
  MD5(buf, filesize, md5s);//using the inbuilt md5sum function call for the hash values
  ...
  printf("\nRemainder - %d\n",md5s[15]%4);
  free(buf);
  return (md5s[15]%4);//returning the modulo for the hash value
}
...
void list(int sock[], struct files *fileList, packet_t packet) {
    ...
    memset(&fileList->parts, 0, sizeof(fileList->parts));//initalizing the values from the structure to 0
    fileList->count = 0;
...
void put(int sock[], packet_t packet) 
{
        struct stat st;
...
fptr = fopen(packet.filename, "rb");
				if (fptr == NULL) {
					...
					size_t f_size = st.st_size;//file size 		//Size of the file
					printf("File Size = %ld\n", f_size);

					md5 = file_md5_counter(packet.filename, f_size, fptr);
//md5 returns modulus for the filename hasg
					float f_data = f_size;
					f_data = (f_data/4);
					int file_part = round(f_data);//filepart divided by 4 and rounded
	...
	packet.file_size = (f_size - file_part*3);//send the whole remaining data in file part 3
	buf = (char *)malloc(packet.file_size);//malloc dynamically for
						}
	else {
